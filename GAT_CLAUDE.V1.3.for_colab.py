...
# 7. Colab 直跑：自动探测常见 Drive 路径并把产物写回同目录
# 8. 训练产物导出：embedding / 映射字典 / 活跃 mask / 配置
# ==========================================
...
model_path = os.getenv("MODEL_PATH", os.path.join(artifact_root, "best_cross_model_v1_3_colab.pth"))
export_dir = os.getenv("EXPORT_DIR", os.path.join(artifact_root, "export_assets_v1_3_colab"))

max_rows = int(os.getenv("MAX_ROWS", "0"))
epochs = int(os.getenv("EPOCHS", "60"))
use_early_stop = os.getenv("USE_EARLY_STOP", "0") == "1"
early_stop_patience = int(os.getenv("EARLY_STOP_PATIENCE", "20"))
early_stop_start_epoch = int(os.getenv("EARLY_STOP_START_EPOCH", "40"))
...
scheduler_start_epoch = int(os.getenv("SCHEDULER_START_EPOCH", "8"))
...
print(
    ...
    f"val_random_ratio={val_random_negative_ratio}, use_early_stop={use_early_stop}"
)
...
for epoch in range(epochs):
    ...
    if use_early_stop and epoch + 1 >= early_stop_start_epoch:
        if val_auc > monitor_best_auc:
            monitor_best_auc = val_auc
            no_improve_cnt = 0
        else:
            no_improve_cnt += 1
            if no_improve_cnt >= early_stop_patience:
                print(
                    f"\n⏹  早停触发：从第 {early_stop_start_epoch} 轮后连续 "
                    f"{early_stop_patience} 个 epoch 无提升，终止训练"
                )
                break
