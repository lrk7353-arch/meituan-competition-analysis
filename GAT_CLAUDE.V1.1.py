...
# 2. 过拟合控制：更保守的学习率、AdamW、更强正则、更长训练+早停
# 3. 监督增强：transition_to 从 80/20 改为 70/30
# 4. Hard Negative Sampling：50% 随机负样本 + 50% 同品类难负样本
# 5. 混合预测头：MLP + 非对称双线性分支，用 sigmoid gate 自适应融合
# 6. 训练产物导出：embedding / 映射字典 / 活跃 mask / 配置
# ==========================================
...
model_path = os.getenv("MODEL_PATH", os.path.join(base_dir, "best_cross_model_v1_0.pth"))
export_dir = os.getenv("EXPORT_DIR", os.path.join(base_dir, "export_assets_v1_0"))
...
scheduler_patience = int(os.getenv("SCHEDULER_PATIENCE", "8"))
message_ratio = float(os.getenv("MESSAGE_RATIO", "0.7"))
hard_negative_ratio = float(os.getenv("HARD_NEGATIVE_RATIO", "0.5"))
...
class CrossAnalysisModel(nn.Module):
    def __init__(self, hidden_channels, metadata, heads=4):
        ...
        # sigmoid(0)=0.5，让 MLP 与 bilinear 分支起点对等
...
val_neg_edges = sample_hard_negative_edges(
    ...
    rng,
    poi_to_cate_idx,
    random_ratio=hard_negative_ratio,
)
...
print("\n🔥 >>>>> V1.0 全图训练（强特征 + 更强正则 + Hard Negative）<<<<< 🔥")
print(
    f"    hidden={hidden_channels}, heads={gat_heads}, lr={learning_rate}, wd={weight_decay}, "
    f"message_ratio={message_ratio}, epochs={epochs}"
)
...
for epoch in range(epochs):
    ...
    print(
        ...
        f"LR: {current_lr:.2e} | Gate: {gate_value:.3f}"
    )
