...
#
# 目标：
# 1. 基于 session 内时序路径构建 category -> next_category 转移
# 2. 统计从 A 转移到 B 后，B 是否在同 session 的后续窗口内产生 ORDER
# 3. 计算 CCR：Cross Conversion Rate
...
output_dir = os.getenv("OUTPUT_DIR", os.path.join(artifact_root, "eda_ccr_outputs"))
...
conversion_window_minutes = float(os.getenv("CONVERSION_WINDOW_MINUTES", "120"))
...
df["event_timestamp"] = df["event_timestamp"].round().astype("int64")
...
transitions["event_timestamp"] = transitions["event_timestamp"].round().astype("int64")
transitions["next_ts"] = transitions["next_ts"].round().astype("int64")
...
if transitions.empty:
    raise RuntimeError("没有提取到有效的一级品类转移，无法计算 CCR 矩阵。")

# ==========================================
# 3. 严格时序版 CCR：A -> B 后，B 是否在后续窗口内产生 ORDER
# ==========================================
order_df = df[df["unified_event"] == "ORDER"][["session_id", "first_cate_name", "event_timestamp"]].copy()
order_df = order_df.rename(columns={"first_cate_name": "next_cate", "event_timestamp": "order_ts"})
order_df["order_ts"] = order_df["order_ts"].round().astype("int64")

if exclude_unknown_category:
    order_df = order_df[order_df["next_cate"] != unknown_category_name].copy()

order_df = order_df.sort_values(["session_id", "next_cate", "order_ts"]).copy()
transitions = transitions.sort_values(["session_id", "next_cate", "next_ts"]).copy()
...
transitions = pd.merge_asof(
    ...
    left_on="next_ts",
    right_on="order_ts",
    by=["session_id", "next_cate"],
    direction="forward",
    allow_exact_matches=True,
)

transitions["order_lag_min"] = (transitions["order_ts"] - transitions["next_ts"]) / 60000.0

if conversion_window_minutes > 0:
    transitions["b_has_order"] = transitions["order_lag_min"].between(
        0,
        conversion_window_minutes,
        inclusive="both",
    ).astype(int)
else:
    transitions["b_has_order"] = transitions["order_ts"].notna().astype(int)
...
pair_stats = (
    ...
    .agg(
        ...
        order_count=("b_has_order", "sum"),
        positive_order_lag_min=(
            "order_lag_min",
            lambda x: float(x[x >= 0].mean()) if np.any(np.isfinite(x[x >= 0])) else np.nan,
        ),
    )
    .reset_index()
)
pair_stats["ccr"] = pair_stats["order_count"] / pair_stats["pair_count"].replace(0, np.nan)
pair_stats = pair_stats.replace([np.inf, -np.inf], np.nan).dropna(subset=["ccr"]).copy()

count_matrix = pair_stats.pivot(index="first_cate_name", columns="next_cate", values="pair_count").fillna(0)
order_matrix = pair_stats.pivot(index="first_cate_name", columns="next_cate", values="order_count").fillna(0)
ccr_matrix = pair_stats.pivot(index="first_cate_name", columns="next_cate", values="ccr").fillna(0)
...
# 5. 导出矩阵与榜单
# ==========================================
summary_path = os.path.join(output_dir, "category_transition_ccr_summary.csv")
count_matrix_path = os.path.join(output_dir, "category_transition_pair_count_matrix.csv")
order_matrix_path = os.path.join(output_dir, "category_transition_order_count_matrix.csv")
ccr_path = os.path.join(output_dir, "category_transition_ccr_matrix.csv")
top_pairs_path = os.path.join(output_dir, "category_transition_top_ccr_pairs.csv")
meta_path = os.path.join(output_dir, "ccr_matrix_metadata.json")

pair_stats.to_csv(summary_path, index=False, encoding="utf-8-sig")
count_matrix.to_csv(count_matrix_path, encoding="utf-8-sig")
order_matrix.to_csv(order_matrix_path, encoding="utf-8-sig")
ccr_matrix.to_csv(ccr_path, encoding="utf-8-sig")
...
metadata = {
    ...
    "category_count": int(len(ccr_matrix.index)),
    ...
    "conversion_window_minutes": float(conversion_window_minutes),
}
...
# 6. 热力图
# ==========================================
heatmap_path = os.path.join(output_dir, "category_transition_ccr_heatmap_topN.png")
title = f"{report_prefix} | 图2 一级品类相邻转移 CCR 热力图"
subtitle = (
    "业务含义：在同一 session 中发生 A→B 后，若目标品类 B 在后续窗口内出现 ORDER，则记为一次转化；"
    "CCR=转化次数/转移次数。"
)
window_desc = (
    f"{conversion_window_minutes:.0f} 分钟"
    if conversion_window_minutes > 0
    else "不限时间窗口"
)
footnote = (
    f"数据口径：{data_date_label} 单日明细日志；基于 session 内相邻一级品类转移，"
    f"并要求目标品类 ORDER 发生在转移之后。"
    ...
    f"展示口径：CCR 判定窗口={window_desc}；仅展示 pair_count≥{plot_min_pair_count} 且按总活跃度排序的 "
    f"Top-{top_n_categories} 源/目标品类；空白表示该品类对样本不足或未进入展示集合。"
)

if plot_pair_stats.empty:
    fig, ax = plt.subplots(figsize=(12, 4.5))
    ax.axis("off")
    ax.text(0.5, 0.5, "过滤后暂无足够样本生成 CCR 热力图", ha="center", va="center", fontsize=15)
    ...
    vmax = max(np.quantile(finite_vals, 0.95), 0.05) if finite_vals.size > 0 else 1.0
    ...
    cbar.set_label("CCR", fontsize=11)
    ...
    if len(heatmap_df.index) <= 10 and len(heatmap_df.columns) <= 10:
        for i in range(len(heatmap_df.index)):
            for j in range(len(heatmap_df.columns)):
                ...
                if pd.notna(value):
                    text_color = "white" if value >= vmax * 0.65 else "#222222"
...
if filtered_pair_stats.empty:
    ...
    preview_df = filtered_pair_stats.head(min(10, len(filtered_pair_stats))).copy()
    print(preview_df.to_string(index=False))

elapsed = time.time() - start_time
print("\n✅ CCR 矩阵计算完成")
print(f"   summary: {summary_path}")
print(f"   pair count matrix: {count_matrix_path}")
print(f"   order count matrix: {order_matrix_path}")
print(f"   ccr matrix: {ccr_path}")
